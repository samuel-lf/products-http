...
export class ProductService {
  ...
  constructor(private http: HttpClient, private DepartService: DepartmentService) { }
  ...
  add(prod: Product): Observable<Product> {
    ...
    return this.http.post<Product>(this.url, { ...prod, departments })
      .pipe(tap((product: Product) => this.productsSubject$.getValue().push({ ...prod, _id: product._id })));
  }

  delete(prod: Product): Observable<any> {
    return this.http.delete<Department>(`${this.url}/${prod._id}`)
      .pipe(tap(() => {
